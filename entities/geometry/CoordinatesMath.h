...
public:
    // Precision constants
    static constexpr double distance_epsilon = 1 / tenToThePowerOf<5>();
...
private:
    ...
    {
        if (0 < deg && deg <= 360)
            return deg; // avoid rounding due to arithmetic ops if within range
    ...
    static std::optional<Coordinates> intersection(const Coordinates &c1, double brng1, const Coordinates &c2, double brng2)
    {
        ...
        if (angular_distance_12 < distance_epsilon)
            return {c1}; // coincident points
        ...
        if (sin(alpha1) * sin(alpha2) < 0)
            return {}; // ambiguous intersection (antipodal?)
    ...
    static auto closestPointOnSegmentAndDistanceApplier(const Coordinates &c, const Coordinates &cA1, const Coordinates &cA2, auto set_return)
    {
        ...
        if (c == cA1)
            return set_return(c, cA1, Meters{0});
        if (c == cA2)
            return set_return(c, cA2, Meters{0});
        ...
        if (relative_bearing_from_cA1 > 90)
            return set_return(c, cA1, Meters{-1}); // Is angle acute?
        ...
        if (d_at >= distance_1sphere_A)
            return set_return(c, cA2, Meters{-1});
